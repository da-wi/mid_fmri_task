...
def start_screen():
	...
	if args.emulation == True:
		...
		ustr = "★"
		text = font.render("Zum Starten \"t\" drücken", 1, FONTCOLOR)
		textpos = text.get_rect()
		textpos.centerx = screen.get_rect().centerx
		...
		font = pygame.font.Font("fonts/seguisym.ttf", 20)
		ustr = "★"
		text = font.render("Bei ★ versuchen schnell \"b\" zu drücken", 1, FONTCOLOR)
		textpos = text.get_rect()
		textpos.centerx = screen.get_rect().centerx
...
def show_feedback(feedback):
	...
	if feedback[2] == '':  # neutral
		...
		if int(feedback[1]) == -2:
			text = font.render("nicht gedrückt", 1, FONTCOLOR)
...
#	}[trialtype]
	if SHAPE_TYPE == 1:
		result = {
			0: "■",  # 'High Loss Avoidance',
			1: "●",  # 'High Gain',
			2: "◧",  # 'Low Loss Avoidance',
			3: "◐",  # 'Low Gain',
			4: "▲"  # 'Neutral'
			#◭
		}[trialtype]
...
# jitter list
jitter_list = [0, 0] * int((num_trials / 2) + 2)
shuffle(jitter_list)
print(jitter_list)

jitter_trial = [2.2, 4.0, 3.1] * int(num_trials / 3)
jitter_trial = random.sample(range(2200, 4000), num_trials)
jitter_trial = [x / 1000.0 for x in jitter_trial]

# a) jitter end via equal distribution
jitter_end = random.sample(range(-100, 100), num_trials)
